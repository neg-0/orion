...
export default function FileBrowser({ setSelectedItem }: { setSelectedItem: React.Dispatch<React.SetStateAction<string>> }) {

  const [files, setFiles] = React.useState<TreeViewBaseItem[] | undefined>();

  const findSelectedItem = (items: TreeViewBaseItem[], id: string): TreeViewBaseItem | undefined => {
  ...
  const handleItemSelectionToggle = (
    ...
    if (isSelected) {
      // Traverse the nested array and find the selected item
      const selectedItem = findSelectedItem(files!, itemId);
  ...
  function fetchFiles() {
    axios.get('/workspace/files', { params: { projectName: 'orion-test' } })
      .then((response) => {
